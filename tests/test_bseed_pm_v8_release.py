...
def test_pm_build_pins_proven_meter_and_next_normal_ota_version():
    ...
    assert "SW_BUILD='1.2.5-bseedv8u3'" in text
    assert "FILE_VERSION_HEX='0x12053006'" in text
    assert "FILE_VERSION_DEC=302329862" in text
...
def test_pm_validator_proves_same_sha_pm_and_ts0726_builds():
    ...
    assert '"board": "OUTLET_BSEED_PM_TS011F"' in text
    assert '"swBuildId": "1.2.5-bseedv8u3"' in text
    assert '"fileVersion": 302329862' in text
