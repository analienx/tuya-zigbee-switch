"""Release-contract tests for the BSEED TS0726 v6 overlay."""
...
OVERLAY = Path("zigbee2mqtt/converters/bseed_ts0726_v6.js")
...
def test_exact_v6_fingerprint_only() -> None:
    js = source()
    assert 'manufacturerName: "iedhxgyi"' in js
    assert 'modelID: "TS0726-3-BS"' in js
    assert 'softwareBuildID: "1.1.6-bseedv6"' in js
...
def test_direct_binding_policy_uses_custom_attribute_not_standard_switch_actions() -> None:
    js = source()
...
def test_fail_closed_static_audit() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "helper_scripts/audit_bseed_ts0726_v6_overlay.py",
            str(OVERLAY),
        ],
        check=True,
        capture_output=True,
...
def test_javascript_syntax_and_behavioral_probes_when_node_is_available() -> None:
    ...
    endpoint_routing = subprocess.run(
        [
            "node",
            "helper_scripts/probe_bseed_ts0726_v6_endpoint_routing.js",
            str(OVERLAY),
        ],
        check=True,
        capture_output=True,
