...
int hlw8012_init(hlw8012_t *dev, hal_gpio_pin_t cf_pin,
    ...
    /* Recovery deliberately restores the hardware-proven predecessor SEL
     * startup sequence while retaining the V8 counter/resource guards above. */
...
static void update_measurement_handler(void *arg) {
     ...
     * to energy. Recovery intentionally contains no V8 no-load suppression. */
    dev->data.energy_acc +=
