...
const TARGET_SW_BUILD = '1.1.6-bseedv6';
...
function verifyContract(definition, exposes, execution) {
    ...
    if ((deviceConfig.access & 2) === 0) {
        die(`device_config must be editable in v6, got access=${JSON.stringify(deviceConfig.access)}`);
    }
    ...
    // Zigbee mutation. It is expected exactly once for the v6 chunk protocol.
    ...
    if (clusterRegistrations[0].args?.[0] !== 'genBasic' ||
        ...
        die(`v6 transport must extend built-in genBasic instead of shadowing ID 0: ${JSON.stringify(clusterRegistrations[0])}`);
    }
    ...
    if (addedCommands.deviceConfigStage?.ID !== 0xf0 || addedCommands.deviceConfigCommit?.ID !== 0xf1) {
        die(`genBasic extension is missing v6 transport commands: ${JSON.stringify(addedCommands)}`);
    }
