 ...
 *   node probe_bseed_ts0726_v56_installed_zhc.js \
...
const TARGET_SW_BUILDS = ['1.1.5-bseedv5', '1.1.6-bseedv6'];
...
function verifyContract(definition, exposes, execution) {
    ...
    if (fpMatches.length !== 2) die(`expected exactly two target fingerprints, got ${fpMatches.length}`);
    const builds = fpMatches.map((fp) => fp.softwareBuildID).sort();
    if (JSON.stringify(builds) !== JSON.stringify([...TARGET_SW_BUILDS].sort())) {
        die(`target fingerprints must require only ${TARGET_SW_BUILDS.join(' / ')}: ${JSON.stringify(builds)}`);
    }
    if (fpMatches.some((fp) => (fp.priority ?? 0) !== 100)) {
        die('both transition fingerprints must use priority 100');
    }
    ...
    if ((deviceConfig.access & 2) === 0) {
        die(`device_config must remain editable in transition, got access=${JSON.stringify(deviceConfig.access)}`);
    }
    ...
    // Zigbee mutation. It is expected exactly once for the V5/V6 chunk protocol.
    ...
    if (clusterRegistrations[0].args?.[0] !== 'genBasic' ||
        ...
        die(`transition transport must extend built-in genBasic instead of shadowing ID 0: ${JSON.stringify(clusterRegistrations[0])}`);
    }
    ...
    if (addedCommands.deviceConfigStage?.ID !== 0xf0 || addedCommands.deviceConfigCommit?.ID !== 0xf1) {
        die(`genBasic extension is missing transition transport commands: ${JSON.stringify(addedCommands)}`);
    }
