...
## 2026-09-19 isolated-module build (actually executed, NOT breaker firmware)

Built an **inert** 1 MiB MG21 application in separate WSL output directory, retaining the previous generic native build. Explicit build parameters: `MCU=EFR32MG21A020F1024IM32`, provisional custom-only `IMAGE_TYPE=60013`, `FILE_VERSION=1`, `CONFIG_STR=LAB;TOQSY2-LAB;M;` (no switches, relays, LEDs, meters or Tongou production GPIO mappings). The generated `silabs_config.h` contains `DEFAULT_CONFIG LAB;TOQSY2-LAB;M;`; the ELF contains this string; SLC and linker completed successfully. This is NOT an implemented native Tongou pin map or a stock identity match. The application/GBL is **not distributed** and no physical device was flashed.

Local Linux artifact: `/home/analienx-agent/tongou-native/build/silabs/tongou_inert_lab_20260919/build/release/tongou_inert_lab_UNSAFE_FOR_BREAKERS.gbl` (194 KiB, SHA-256 `83bfbc553dfefcfc0ecb7d721db60f75c0f053506ce9b5e180b49a46b93464bf`). Compiled `.s37` SHA-256 `0fbf0a7a7b5439550d400f8b5f920096610c4cc586a1d72b897766d5bde916c2`. Generated `.gbl` successfully round-tripped with Commander `gbl parse --app`. Commander appinfo reports application v1, **no signature**, **no product ID** or certificate. These signatures are NOT a claim about the unknown installed stock bootloader.

There are pre-existing ARM build warnings elsewhere (32-bit `printf` ABI, generated ZCL length narrowing, and `noreturn` annotation) requiring separate review before hardware. The reported ~194 KiB is compressed GBL file size, **not** full flash usage, and successful native linkage is not evidence of operating RAM margin.

**Release blocker:** PR #37's two legacy BSEED exact-byte equivalence gates (`build-clients` and `build-nonpm-canary`) still failed as of this build despite passing host tests, code formatting and separate Router compilation. No bypass or relaxation of those checks is authorized; keep the PR draft and the inert GBL out of OTA indexes until the baseline discrepancy has been understood and removed.
