...
/**
 * BSEED TS0726-3-BS v4 canary overlay.
 *
 * Deployment:
 *   - keep the exact historical fleet converter (ef79...) unchanged;
 *   - load this file beside it;
 *   - this definition wins only for forward firmware 1.1.4-bseedv4.
 *
...
const indicatorBehavior = (name, endpointName) =>
    enumLookup({
        ...
        description:
            "same: panel LED follows logical relay state; opposite: inverse logical state; " +
            "manual: Indicator LED state is controlled separately. On v4 canonical firmware " +
            "this controls only the panel LED, never the mains relay.",
        entityCategory: "config",
    });
...
const indicatorState = (name, endpointName) =>
    binary({
        ...
        description: "Manual panel-LED state; used only when Indicator LED behavior is manual.",
        access: "ALL",
        entityCategory: "config",
    });
...
const deviceConfigReadOnly = (name, endpointName) =>
    withExposeLabel(
        text({
            name,
            endpointName,
            ...
            description:
                "Read-only canary view of the Romasku hardware pin map. Firmware migration owns this " +
                "value during BSEED v4. Editing a pin map can disconnect buttons/indicators or switch " +
                "mains unexpectedly and may require recovery firmware; writes are disabled here.",
...
module.exports = [
    {
        ...
        model: "EC-GL86ZPCS31",
        vendor: "Tuya-custom",
        description: "BSEED Echo Click / Scale 3-gang — Romasku v4 canary",
        extend: [
            deviceEndpoints({
                endpoints: {
                    switch_left: 1,
            ...
            deviceConfigReadOnly("device_config", "switch_left"),
        ],
