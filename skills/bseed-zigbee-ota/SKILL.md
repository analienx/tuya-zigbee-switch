...
## 2026-09-21: Runtime version truth versus stale Zigbee2MQTT cache

On `KitchenSocketRight` (`0xa4c138075cd16ed4`), a live Zigbee ZCL read-response from endpoint 1, cluster `genBasic` (0x0000), attribute `swBuildId` (0x4000), returned **`1.2.5-bseedv8u5-rc1`**, status 0, type 0x42. A separate live `genOta.currentFileVersion` read returned **302329869**, confirming the rc1 image is executing after a mains power cycle. The `bridge/devices` and persisted database continued to report the old `v8u4` build after one further successful target-only interview. Do not misclassify this as an unactivated OTA or reflash to change the display.

Cause/compatibility: the old build ID is 15 octets; the rc1 label is **19 octets**. Zigbee-herdsman defines `genBasic.swBuildId.maxLen = 16` and checks `maxLen` in `processAttributePostRead`. Its current Zigbee2MQTT `reInterview()` explicitly calls `zh.interview(true)`; do not claim the repeated interview merely skipped a cached field. A raw wire-level string can therefore be correct yet fail higher-level validation and leave stale metadata. This applies to every firmware variant, not only PM sockets. `src/zigbee/basic_cluster.c` now rejects build-time version IDs exceeding 16 octets; existing rc1/rc2 binary identities remain immutable. A subsequent fix requires a **new build label of at most 16 octets and a distinct OTA fileVersion/hash**, approved as a separately scoped canary; NEVER silently rename/rebuild an existing OTA.

Do not interpret an empty Zigbee2MQTT `Read result of 'genBasic': {}` as an empty raw ZCL response. Use a bounded, target/NWK/endpoint-pinned raw read log to distinguish the two and immediately restore temporary debug levels. The standard interview-only mode is fail-closed on a stale inventory build; it must not reflash or mark hardware accepted. No live Zigbee2MQTT database edits, network-wide restart, relay command, or energy-counter write are authorized by this finding.

Endpoint-2 OnOff raw read is OFF, consistent with Right's saved power-on behavior OFF after reboot; switch logical endpoint-1 `state: ON` is not the physical relay. Metering endpoint-1 raw summation is zero with divisor 1000; the Sep 20 *cached* preflash database held raw 1172 with divisor 100, but a contemporaneous independent preflash raw ZCL reading was not preserved. Preserve that historic counter and scaling evidence; do not inject 11.72 kWh into NVM or claim a proven physical energy loss from cached evidence alone.
