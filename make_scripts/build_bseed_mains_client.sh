...
if [[ "${BSEED_ANTIBRICK_RC:-0}" == "1" || "${BSEED_ANTIBRICK_RC:-0}" == "2" ]]; then
    ...
    *) echo 'ERROR: anti-brick RC limited to verified BSEED sockets' >&2; exit 2 ;;
    ...
    if [[ "${BSEED_ANTIBRICK_RC}" == "2" ]]; then
        case "$TARGET" in
        pm) SW_BUILD='1.2.5-bseedcli8'; FILE_VERSION_HEX='0x12053011'; FILE_VERSION_DEC=302329873;
            DEFAULT_OUT='build/bseed-ts011f-pm-client-antibrick-r2' ;;
        nonpm) SW_BUILD='1.1.2-bseedcli7'; FILE_VERSION_HEX='0x11023013'; FILE_VERSION_DEC=285356051;
            DEFAULT_OUT='build/bseed-ts011f-nonpm-client-antibrick-r2' ;;
        esac
    fi
fi
