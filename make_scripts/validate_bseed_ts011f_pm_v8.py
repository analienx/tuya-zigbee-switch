#!/usr/bin/env python3
"""Reproducibility gate for the BSEED TS011F-BS-PM recovery target.

The recovery keeps the V8/Telink platform and OTA lineage while restoring the
hardware-proven predecessor PM sampling semantics. This validator builds only;
...
FOCUSED = [
    "tests/test_pm_recovery_contract.py",
...
def run(cmd: list[str]) -> dict[str, object]:
    ...
    result = {"command": cmd, "exitCode": proc.returncode, "stdout": proc.stdout, "stderr": proc.stderr}
...
def load_manifest(relative: str) -> dict[str, object]:
    path = ROOT / relative
    if not path.is_file(): raise SystemExit(f"missing manifest: {relative}")
    return json.loads(path.read_text(encoding="utf-8"))


def require_values(name: str, manifest: dict[str, object], expected: dict[str, object]) -> None:
    for key, value in expected.items():
        if manifest.get(key) != value:
            raise SystemExit(f"{name} manifest mismatch for {key}: {manifest.get(key)!r} != {value!r}")
...
def main() -> int:
    ...
    for script in (pm_build, ts0726_build):
        if b"\r\n" in script.read_bytes(): raise SystemExit(f"{script.name} contains CRLF")

    head = str(run(["git", "rev-parse", "HEAD"])["stdout"]).strip()
    dirty = str(run(["git", "status", "--porcelain"])["stdout"]).strip()
    if dirty: raise SystemExit("working tree must be clean before reproducibility validation")
    ...
    steps.append(run([sys.executable, "helper_scripts/check_image_types.py", "device_db.yaml", "--changed-base", PM_INTEGRATION_BASE]))
    ...
    pm_manifest_path = "build/bseed-ts011f-pm-recovery/manifest.json"
    pm_manifest = load_manifest(pm_manifest_path)
    require_values("PM recovery", pm_manifest, {
        ...
        "fileVersion": 302329859,
        "manufacturerCode": 4417,
        "imageType": 43556,
        "canonicalConfig": "b28wrpvx;TS011F-BS-PM;LC3;SB5u;RD2;IB4;M;",
    })
    meter = pm_manifest.get("meter")
    if not isinstance(meter, dict): raise SystemExit("PM manifest missing meter object")
    for key, value in {
        "type": "BL0937", "cf": "PA1", "cf1": "PC2", "sel": "PB1",
        "voltageMultiplier": 161460, "currentMultiplier": 144679,
        "powerMultiplier": 16989, "protectionEnabled": True,
        "samplingSemantics": "8b8cc492-compatible", "noLoadSuppression": False,
        "selStartup": "legacy-init-then-set",
    }.items():
        if meter.get(key) != value: raise SystemExit(f"PM recovery meter manifest mismatch for {key}")

    ts0726_manifest_path = "build/bseed-ts0726-v8/manifest.json"
    ts0726_manifest = load_manifest(ts0726_manifest_path)
    require_values("TS0726", ts0726_manifest, {
        "sourceCommit": head, "sourceDirty": False,
        "swBuildId": "1.1.8-bseedv8", "fileVersion": 285356042,
    ...
    if final_dirty: raise SystemExit(f"validation modified tracked/unignored files:\n{final_dirty}")

    output = {
        "status": "PASS", "sourceCommit": head, "integrationBase": PM_INTEGRATION_BASE,
        "focusedTests": FOCUSED, "pmManifest": pm_manifest_path,
        "pmArtifacts": pm_manifest["artifacts"], "ts0726Manifest": ts0726_manifest_path,
        "ts0726Artifacts": ts0726_manifest["artifacts"],
        "steps": [{"command": item["command"], "exitCode": item["exitCode"]} for item in steps],
